...
describe('MIDI note formatting and sanitizing', () => {
  it('uses scientific note names and a typographic range separator', () => {
    expect(formatMidiNote(21)).toBe('A0')
    expect(formatMidiNote(36)).toBe('C2')
    expect(formatMidiNote(60)).toBe('C4')
    expect(formatMidiNote(61)).toBe('C♯4')
    expect(formatMidiNote(108)).toBe('C8')
    expect(formatMidiNote(Number.NaN)).toBe('—')
    expect(formatMidiRange(36, 96)).toBe('C2–C7')
    expect(getKeyboardConfigLabel(DEFAULT_KEYBOARD_CONFIG)).toBe('Yamaha PSR-E383')
    expect(getKeyboardConfigLabel({ preset: 'detected', startMidi: 28, endMidi: 103 })).toBe('Detected · 76 keys')
  })
  ...
  it('clamps, rounds, and orders custom endpoints', () => {
    expect(sanitizeKeyboardConfig({
      ...
      startMidi: 140,
      endMidi: 20.6,
    })).toEqual({ preset: 'custom', startMidi: 21, endMidi: 127 })
  })
...
describe('practice range resolution', () => {
  it('keeps complete physical, custom, and detected ranges regardless of song notes', () => {
    ...
    expect(resolvePracticeRange([60], { preset: 'custom', startMidi: 41, endMidi: 89 })).toEqual([41, 89])
    expect(resolvePracticeRange([60], { preset: 'detected', startMidi: 28, endMidi: 103 })).toEqual([28, 103])
  })
